...
# Ensure API key exists (use dev default if none set in the shell)
...
# 2) Backup and remove any old SQLite DB
...
echo "Running go mod tidy + build..."
go mod tidy
go build ./...
